...
def check_environment():
    # Check if 'google.colab' is in the system modules
    if 'google.colab' in sys.modules:
        return "Running in Google Colab"
    else:
        return "Running on a local PC"
...
img_rows = x_train.shape[1] #高
img_cols = x_train.shape[2] #寬
channels = x_train.shape[3] #顏色

# create saved_images folder 建立文件夾
...
kernel_size = 3
latent_dim = 256
# encoder/decoder number of CNN layers and filters per layer 這裡是設定CNN的層數
...
# 模型
# 儲存 model 和 weight
fileName="灰階轉彩色autoencoder1"
file_model = fileName+'.json'
with open(file_model, "w") as json_file:
    json_file.write(autoencoder.to_json())
file_decoder_model = fileName+'_decoder.json'
with open(file_decoder_model, "w") as json_file:
