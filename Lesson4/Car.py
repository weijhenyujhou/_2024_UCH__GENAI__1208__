...
max = 100000
category=101
fileName ="cats_vs_dogs"
if(path.exists(fileName+"_x.csv") and path.exists(fileName+"_y.csv")   ):
    print("Find"+fileName+"_x.csv and "+fileName+"_y.csv")
else:
    # batch_size=-1 to get the full dataset in NumPy arrays from the returned tf.Tensor object
    train , info  = tfds.load(name="cats_vs_dogs", split=tfds.Split.TRAIN, batch_size=1,with_info=True )
    print(info)
    if path.exists(fileName+"_x.csv"):
        os.remove(fileName+"_x.csv")
    if path.exists(fileName+"_y.csv"):
        os.remove(fileName+"_y.csv")
    fx = open(fileName+"_x.csv", "a+")
    fy = open(fileName+"_y.csv", "a+")
    i=0
...
if(path.exists(fileName+"_x.csv") and path.exists(fileName+"_y.csv")   ):
    # image=np.fromstring(image, sep=',')
    # label=np.fromstring(label, sep=',')
    x = np.loadtxt(fileName+'_x.csv', delimiter=',')
    y = np.loadtxt(fileName+'_y.csv', delimiter=',')
    print(x.shape)
    print(y.shape)
    x = x.reshape(x.shape[0], size , size, 3)
    y = y.reshape(y.shape[0],1)
